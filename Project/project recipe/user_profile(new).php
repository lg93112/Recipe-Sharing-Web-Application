...
<div class="container">
  <div class="row">
    ...
    <p>Similar Users also like:</p>
    <?php
     $result = json_decode(exec('python recommender_system.py'), true);
     $recipeid=$result['recipesid'];
     $ids = join("','",$recipeid);
     $sql = mysql_query("SELECT * FROM Recipes WHERE recipeId IN ('$ids')");
     while ($row = mysql_fetch_assoc($sql)){
         echo "<div>";
         echo $row['title'] . "&nbsp"."<a href='view_recipe.php?id=$row[recipeId]&user=$current_user'>view</a>";
         echo "</div>";
     }
    ?>
    </div>
